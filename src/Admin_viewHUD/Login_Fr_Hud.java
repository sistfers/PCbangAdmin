...
public class Login_Fr_Hud extends JFrame implements ActionListener {
	...
	public Login_Fr_Hud() {
		...
		// 이미지 받아오기			
		img = new ImageIcon(getClass().getClassLoader().getResource("login.png"));
		...
		Font f = new Font("HY울릉도L", Font.BOLD, 20);
		// 첫번째 텍스트창 731, 399
		...
		bt = new JButton(); // 587 458 에
																// 위치해야한다.
		bt.setBorderPainted(false);
		bt.setFocusPainted(false);
		bt.setContentAreaFilled(false);
		/** 이게 레알이네: 버튼에 이미지만 보여주는 기능 */
		bt.setBounds(710, 560, 170, 48);
	...
	public void actionPerformed(ActionEvent e) { // 로그인 처리 부
		...
		if (tf.getText().equals("") || tf2.getText().equals(""))// 아이디 비번입력했는지 검사
		{
				...
				
			{
				...
				JOptionPane.showMessageDialog(null, "로그인에 성공하였습니다.", "로그인 성공",
						JOptionPane.INFORMATION_MESSAGE);
				vc.mainFrameHUD();
				vc.accountDao.log_id =tf.getText();
				this.dispose();
